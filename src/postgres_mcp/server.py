...
from .oauth_server import EmbeddedOAuthServer
...
async def main():
    ...
    parser.add_argument(
        "--embed-oauth",
        action="store_true",
        help="Run an embedded OAuth2 server (RS256) alongside and use it for auth",
    )
    parser.add_argument(
        "--embed-oauth-host",
        type=str,
        default="localhost",
        help="Host for the embedded OAuth2 server (default: localhost)",
    )
    parser.add_argument(
        "--embed-oauth-port",
        type=int,
        default=9001,
        help="Port for the embedded OAuth2 server (default: 9001)",
    )
    parser.add_argument(
        "--embed-oauth-client-id",
        type=str,
        help="Client ID for embedded OAuth2 server (optional; autogenerated if omitted)",
    )
    parser.add_argument(
        "--embed-oauth-client-secret",
        type=str,
        help="Client secret for embedded OAuth2 server (optional; autogenerated if omitted)",
    )
    parser.add_argument(
        "--embed-oauth-default-scopes",
        type=str,
        help="Comma-separated default scopes granted by embedded OAuth2 server",
    )
    ...
    embedded_oauth: EmbeddedOAuthServer | None = None
    try:
        # Optionally start an embedded OAuth2 server, and default auth settings to it
        if args.embed_oauth:
            audience_for_embedded = args.oauth_audience or "postgres-mcp"
            embedded_oauth = EmbeddedOAuthServer.from_env_or_args(
                host=args.embed_oauth_host,
                port=args.embed_oauth_port,
                audience=audience_for_embedded,
                default_scopes_csv=args.embed_oauth_default_scopes,
                client_id=args.embed_oauth_client_id,
                client_secret=args.embed_oauth_client_secret,
            )
            # Point auth to the embedded server unless explicitly overridden
            args.oauth_issuer = args.oauth_issuer or embedded_oauth.issuer
            args.oauth_jwks_url = args.oauth_jwks_url or embedded_oauth.jwks_url

            # Start embedded OAuth in the background
            asyncio.create_task(embedded_oauth.serve())
