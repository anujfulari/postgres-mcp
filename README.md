...
##### Embedded OAuth2 Server (no OAUTH_HS256_SECRET)

For local setups without an external IdP, you can run a minimal OAuth2 server (RS256) inside this process and use it for authorization. This avoids `OAUTH_HS256_SECRET` and uses JWKS (public keys) instead.

Start the server with embedded OAuth and set an audience:
```bash
postgres-mcp \
  --transport sse \
  --sse-host 0.0.0.0 --sse-port 8000 \
  --embed-oauth \
  --oauth-audience postgres-mcp
# Optional flags:
#   --embed-oauth-host localhost
#   --embed-oauth-port 9001
#   --embed-oauth-client-id <id>
#   --embed-oauth-client-secret <secret>
#   --embed-oauth-default-scopes "db.read,db.tune"
```

On startup, a client is registered. If you didn’t pass credentials, an ID/secret are autogenerated and printed once in logs. Obtain a token via client_credentials:
```bash
curl -s -X POST "http://localhost:9001/oauth/token" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "grant_type=client_credentials" \
  -d "client_id=<printed-or-configured-id>" \
  -d "client_secret=<printed-or-configured-secret>" \
  -d "scope=db.read" | jq
```

Use the returned `access_token` as the `Authorization: Bearer <token>` header when connecting to the MCP server.
